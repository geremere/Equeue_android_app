...
class ActiveQueueRootFragment : Fragment() {
    ...
    private fun observeQueueLoaded() {
        activeQueueViewModel.queueResult.observe(viewLifecycleOwner) { result ->
            result
                .onSuccess {
                    activeQueueViewModel.setQueue(it)
                    val ft = parentFragmentManager.beginTransaction()
                    val activeQueue = ActiveQueueFragment()
                    ft.replace(R.id.active_queue_root, activeQueue)
                    ft.commit()
                }
                .onFailure {
                    val ft = parentFragmentManager.beginTransaction()
                    val activeQueue = NotActiveQueueFragment()
                    ft.replace(R.id.active_queue_root, activeQueue)
                    ft.commit()
                }
        }
